...
def defaultStateFuc(timeseries, timeseries_curser, previous_state=None, action=None):
    """
    Improved RNNBinaryStateFuc: returns a windowed state of shape [n_steps, 2].
    Pads early states if the cursor is less than n_steps.
    """
    n_steps = 25  # This must match your model setting
    values = timeseries['value']

    if timeseries_curser < n_steps:
        # Pad beginning with the first value if not enough history
    ...
    if previous_state is None or len(previous_state) != n_steps:
        # Create state from scratch if no previous state
        state = []
        for i in range(n_steps - 1):
            state.append([values.iloc[timeseries_curser - n_steps + i], 0])
        state.append([values.iloc[timeseries_curser], 1])
        return np.array(state, dtype='float32')

    # Use previous state to roll and add new action-indicated value
    state = np.concatenate((previous_state[1:], [[values.iloc[timeseries_curser], 1]]))
    return np.array([  # binary branching for Q-learning
        np.concatenate((previous_state[1:], [[values.iloc[timeseries_curser], 0]])),
        state
    ], dtype='float32')
...
class EnvTimeSeriesfromRepo():
    ...
    def step(self, action):
        ...
        if self.timeseries_curser >= self.timeseries['value'].size:
            ...
            if state is None:
                print("⚠️ WARNING: state is None at t =", self.timeseries_curser)
                state = np.array([self.timeseries_states, self.timeseries_states])
                done = 1
